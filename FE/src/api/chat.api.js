...
const chatApi = () => ({
    ...
    createConversation: async(id, {content}) => apiPHP.post(`${ApiConstant.chat.createConversation}/${id}`,{
        content
    }),
    getMessages: async(id) => apiPHP.get(`${ApiConstant.chat.getMessageById}/${id}`),
    sendMessages: async({id, content}) => apiPHP.post(ApiConstant.chat.sendMessages,{
        id,
        content
    })
});
